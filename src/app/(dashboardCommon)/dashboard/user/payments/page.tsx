...
import PaymentUserTable from "@/component/Table/PaymentUserTable";
import Pagination from "@/QueryBuilders/Pagination";
import { useFetchUserPaymentQuery } from "@/redux/api/paymentApi";
...
const page = () => {
  ...
  const id = user?.id;
  const { error, data, refetch, isLoading } = useFetchUserPaymentQuery({
    id,
    filters,
  });
  if (isLoading) {
          ...
          <div className="align-middle inline-block min-w-full shadow overflow-hidden bg-white shadow-dashboard px-8 pt-3 rounded-bl-lg rounded-br-lg">
            ...
            <table className="min-w-auto">
              ...
              <tbody className="bg-white">
                {data?.data?.result?.map((payment) => (
                  <PaymentUserTable key={payment?._id} payment={payment} />
                ))}
              </tbody>
            </table>
